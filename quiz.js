(async function () {
  ...
  function renderAnswersView(obj) {
    mount.innerHTML = '<h2>All Answers</h2>'; const list = document.createElement('ol'); const qs = data.questions;
    for (let i = 0; i < qs.length; i++) {
      const q = qs[i]; const myAns = q.authorAnswer; const myGuess = q.authorGuessForAbby; const abbyAns = obj.answers[i]; const abbyGuess = obj.guesses[i]; const correct = (abbyGuess === myAns); const match = (abbyAns === myAns); const iGuessedRight = (myGuess === abbyAns);
      const li = document.createElement('li'); li.innerHTML = `<p><strong>${q.text}</strong></p><p>You answered: <em>${q.options[abbyAns] ?? "—"}</em></p><p>You guessed I answered: <em>${q.options[abbyGuess] ?? "—"}</em> ${correct ? "✅" : "❌"}</p><p>What I actually answered: <strong>${q.options[myAns]}</strong></p><p><small>I guessed you would pick: <em>${q.options[myGuess]}</em></small></p><hr/><p><small>Match? ${match ? "💖 Yes!" : "Not this time"} · Did I guess yours right? ${iGuessedRight ? "🔮 Yes!" : "Nope 😅"}</small></p>`; list.appendChild(li);
    }
    mount.appendChild(list); nav.style.display = 'none'; const actions = document.createElement('div'); actions.className = 'actions'; actions.innerHTML = `<a class="btn" href="history.html">Back</a> <a class="btn" href="index.html">Home</a>`; mount.appendChild(actions);
  }
